defmodule PurehavenWeb.AdminSettingsLive do
  ...
  def render(assigns) do
    ...
    <div class="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-2xl mx-auto bg-white shadow-md rounded-lg p-8 space-y-12">
        <div class="text-center">
          <h2 class="text-3xl font-bold text-gray-800">Account Settings</h2>
          <p class="mt-2 text-sm text-gray-500">Manage your email and password</p>
        </div>

        <!-- Email Update Form -->
        <div>
          <h3 class="text-xl font-semibold text-gray-700 mb-4">Change Email</h3>
          <.simple_form
            for={@email_form}
            id="email_form"
  ...
  def mount(%{"token" => token}, _session, socket) do
    socket =
      case Accounts.update_admin_email(socket.assigns.current_admin, token) do
        ...
        :error ->
          put_flash(socket, :error, "Email change link is invalid or has expired.")
      end
